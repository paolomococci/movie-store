...
public class RentalRepoMockMvcParametrizedTests {
	...
	@Test
	@Order(1)
	@DisplayName("create a record into rental's data table")
	void createTest() throws 
			Exception, 
			DataIntegrityViolationException, 
			ConstraintViolationException {
		MvcResult mvcResult = mockMvc
			.perform(post("/rentals").content(RENTAL_TEST_STRING))
	...
	@DisplayName("read a record from the rental's data table identifying it from the uri")
	...
	@ParameterizedTest
	@MethodSource("initUri")
	@DisplayName("update a record into the rental's data table identifying it from the uri")
	...
	@ParameterizedTest
	@MethodSource("initUri")
	@DisplayName("partial update a record into the rental's data table identifying it from the uri")
	...
	@ParameterizedTest
	@MethodSource("initUri")
	@DisplayName("delete a record from the rental's data table identifying it from the uri")
	...
	@ParameterizedTest
	@MethodSource("initUri")
	@DisplayName("try to read a deleted record from the rental's data table identifying it from the uri")
	...
	private static void setUri(URI uri) {
		RentalRepoMockMvcParametrizedTests.uri = uri;
	}
	
	private static Stream<String> initUri() {
		return Stream.of(RentalRepoMockMvcParametrizedTests.getUri().getPath());
	}
}
