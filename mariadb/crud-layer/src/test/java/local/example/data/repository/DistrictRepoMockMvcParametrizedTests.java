...
public class DistrictRepoMockMvcParametrizedTests {
	...
	void createTest() 
		...
		MvcResult mvcResult = mockMvc
			.perform(post("/districts").content(DISTRICT_TEST_STRING))
	...
	void patchTest(String uri) 
		...
		mockMvc.perform(patch(uri)
			.content("{\"name\":\"GrayPoint\"}"))
			.andExpect(status().isNoContent()); 
		mockMvc.perform(get(uri))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.name").value("GrayPoint"));
	}
	...
	private static void setUri(URI uri) {
		DistrictRepoMockMvcParametrizedTests.uri = uri;
	}
	
	private static Stream<String> initUri() {
		return Stream.of(DistrictRepoMockMvcParametrizedTests.getUri().getPath());
	}
}
